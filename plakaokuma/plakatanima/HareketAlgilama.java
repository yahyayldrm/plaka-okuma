...
public class HareketAlgilama extends Thread {
	//***** Görüntü işleme alanınnı karartıyor ...
	...
	public void run() {
		...
		while (true) {
			...
			resultImg = img.clone();
			 // aldığımız fotoğrafın genişliğini canalını ve yükseklik ve enini oluşturuyor.
			IplImage resize2 = IplImage.create(640, 360, resultImg.depth(), resultImg.nChannels());
			//yeni oluşturulan görüntüyü yakınlaştırma işlevini yerine getiriyor.
			cvResize(resultImg, resize2);
			// görüntü gri tonlamaya dönüştürülüyor.
			...
			opencv_imgproc.cvCvtColor(resultImg, gray, CV_BGR2GRAY); // renkli görüntüyü gri tonlamaya çeviriyor.
			 //Iplımage üzerinden kanal yükseklik gibi özellikleri yarattıktan sonra  videolabel atıyoruz
			myImageIcon = new ImageIcon(convertTobuffer.convert(converterIPL.convert(resize2)));  

			AnaPencere.videoGirisLabel
					.setIcon(myImageIcon);
			//Araç giriş çıkış esnasında 5 saniye bekletiyor bariyer açma süresi
			...
			if(zamanFarki > 5000) {
				...
				//Görüntü bindirmesi yapılıyor görüntünün belirli kısımları alınıyor 
				...
				//ıplımageden  mat çevirme işlemi yapılıyor
				...
				mog.setNMixtures(3);
				//arka plandaki karenin son değere göre değiştiği alan 
				mog.apply(crrntFrame, foreground, 0.09); 	
               //Bu alan görüntü işleme ekranını karartıyor...
	...
	public static void plakaBul() {
		...
		try {
			// ıpl görüntüsü img nesnesine atanıyor 
			BufferedImage img = convertTobuffer.convert(converterIPL.convert(resultImg));
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			//img nesnesine atadığımız fotoğrafı png formatında baos nesnesine yazdırıyor.
			...
			//Plaka okuma işlemi recognize alanında yapılıyor.
			AlprResults results = alpr.recognize(imageInByte);
			for (AlprPlateResult result : results.getPlates()) {
				//plaka bilgisi alındı
				if (result.getBestPlate().getCharacters() != null) {
					...
					if (!getKarakterYeni().equals(getKarakterEski()) & (uzunluk > 6)) {  // plakanın uzunluğu altıdan büyükssse giriyor
						...
						if(!hataDurumu) {
							//if AracGiriyorMu true ise AracGiris değilse AracCikis
